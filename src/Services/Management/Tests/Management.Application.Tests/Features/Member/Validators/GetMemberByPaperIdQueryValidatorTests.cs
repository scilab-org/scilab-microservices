using Management.Application.Features.Member.Queries;
using Management.Application.Tests.Common;

namespace Management.Application.Tests.Features.Member.Validators;

public sealed class GetMemberByPaperIdQueryValidatorTests : BaseTest
{
    private readonly GetMemberByPaperIdQueryValidator _validator = new();

    [Fact]
    public async Task Validate_ShouldPass_WhenQueryIsValid()
    {
        // Arrange
        var query = new GetMemberByPaperIdQuery(Guid.NewGuid(), Guid.NewGuid());

        // Act
        var result = await _validator.ValidateAsync(query, CancellationToken);

        // Assert
        result.IsValid.Should().BeTrue();
    }

    [Fact]
    public async Task Validate_ShouldFail_WhenPaperIdIsEmpty()
    {
        // Arrange
        var query = new GetMemberByPaperIdQuery(Guid.Empty, Guid.NewGuid());

        // Act
        var result = await _validator.ValidateAsync(query, CancellationToken);

        // Assert
        result.IsValid.Should().BeFalse();
        result.Errors.Should().Contain(e => e.ErrorMessage == MessageCode.PaperIdIsRequired);
    }

    [Fact]
    public async Task Validate_ShouldFail_WhenUserIdIsEmpty()
    {
        // Arrange
        var query = new GetMemberByPaperIdQuery(Guid.NewGuid(), Guid.Empty);

        // Act
        var result = await _validator.ValidateAsync(query, CancellationToken);

        // Assert
        result.IsValid.Should().BeFalse();
        result.Errors.Should().Contain(e => e.ErrorMessage == MessageCode.UserIdIsRequired);
    }
}
