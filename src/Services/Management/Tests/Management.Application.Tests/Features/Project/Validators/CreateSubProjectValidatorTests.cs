using Management.Application.Features.Project.Commands;
using Management.Application.Tests.Common;

namespace Management.Application.Tests.Features.Project.Validators;

public sealed class CreateSubProjectValidatorTests : BaseTest
{
    private readonly CreateSubProjectValidator _validator = new();

    [Fact]
    public async Task Validate_ShouldPass_WhenCommandIsValid()
    {
        // Arrange
        var dto = new CreateSubProjectDto { PaperId = Guid.NewGuid(), Name = "Sub Project" };
        var command = new CreateSubProjectCommand(Guid.NewGuid(), dto, "admin");

        // Act
        var result = await _validator.ValidateAsync(command, CancellationToken);

        // Assert
        result.IsValid.Should().BeTrue();
    }

    [Fact]
    public async Task Validate_ShouldFail_WhenProjectIdIsEmpty()
    {
        // Arrange
        var dto = new CreateSubProjectDto { PaperId = Guid.NewGuid() };
        var command = new CreateSubProjectCommand(Guid.Empty, dto, "admin");

        // Act
        var result = await _validator.ValidateAsync(command, CancellationToken);

        // Assert
        result.IsValid.Should().BeFalse();
        result.Errors.Should().Contain(e => e.ErrorMessage == MessageCode.ProjectIdIsRequired);
    }

    [Fact]
    public async Task Validate_ShouldFail_WhenPaperIdIsEmpty()
    {
        // Arrange
        var dto = new CreateSubProjectDto { PaperId = Guid.Empty };
        var command = new CreateSubProjectCommand(Guid.NewGuid(), dto, "admin");

        // Act
        var result = await _validator.ValidateAsync(command, CancellationToken);

        // Assert
        result.IsValid.Should().BeFalse();
        result.Errors.Should().Contain(e => e.ErrorMessage == "PAPER_IDS_REQUIRED");
    }
}
